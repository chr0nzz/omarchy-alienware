...
func ParseWriteMode(s string) (WriteMode, error) {
	...
	default:
		return 0, fmt.Errorf("elc: unknown transport %q, want output, write or feature", s)
	}
}
...
var ErrDeviceTimeout = fmt.Errorf("elc: device did not report ready before the timeout")
...
func (d *Device) send(frame []byte) error {
	...
	default:
		return fmt.Errorf("elc: unknown write mode %v", d.writeMode)
	}
	return err
}
...
func (d *Device) SelectZones(loop bool, zoneIDs []uint8) error {
	if len(zoneIDs) > MaxSelectZonesPerFrame {
		return fmt.Errorf("elc: %d zones exceeds the %d a single select-zones frame can carry", len(zoneIDs), MaxSelectZonesPerFrame)
	}
...
func (d *Device) AddAction(phases []Phase) error {
	if len(phases) > MaxActionPhasesPerFrame {
		return fmt.Errorf("elc: %d phases exceeds the %d a single add-action frame can carry", len(phases), MaxActionPhasesPerFrame)
	}
...
func (d *Device) SetZoneColor(r, g, b uint8, zoneIDs []uint8) error {
	...
	if err := d.SetColorZones(r, g, b, zoneIDs); err != nil {
		return err
	}
	return d.Apply()
}
